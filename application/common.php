...
//
//    //生成二维码图片
//    $QR = $code->png($value,false,$errorCorrectionLevel, $matrixPointSize, 2);
////        $filename = 'qrcode/'.microtime().'.png';
//    $filename = 'qrcode.png';
////        $QR = $code->png($value,$filename , $errorCorrectionLevel, $matrixPointSize, 2);
//
//    $QR = $filename;                //已经生成的原始二维码图片文件
//
//
//    $QR = imagecreatefromstring(file_get_contents($QR));
//    $qq = imagepng($QR, 'qrcode.png');
//    imagedestroy($QR);
//    die;
//}
 ...
 *
 */
function DBdate($table,$map,$limit=10,$page=1,$order){
    $DB = Db::name($table)->where($map)->limit($limit)->page($page)->order($order)->select();
 ...
 *
 */
function DBdateone($table,$map){
    $DB = Db::name($table)->where($map)->find();
 ...
 * zzy
 */
function DBsaves($table,$param,$filter=''){
    //比较数组键名，返回第一个数组的差/*zzy*/   俗称去掉不需要的字段名
    if($filter!=''){
        $map = array_diff_key($param,$filter);

    }else{
        $map = $param;

    }
    $DB = Db::name($table)->strict(false)->update($map);
    if (false === $DB) {
 ...
 * zzy
 */
function DBdel($table,$id,$manid=''){
    if($manid!=''){
        $flag = Db::name($table)->strict(false)->where($manid,$id)->delete();

    }else{
        $flag = Db::name($table)->strict(false)->delete($id);
 ...
 * zzy
 */
function DBadd($table,$param,$manid='',$filter=''){
    //比较数组键名，返回第一个数组的差/*zzy*/   俗称去掉不需要的字段名
    if($filter!=''){
        $map = array_diff_key($param,$filter);
 ...
 * @return array
 */
function DBaddzzy($table,$param,$filter){
...
function DBcount($table,$where=''){
    $flag = Db::name($table)->where($where)->count();

    if (false === $flag) {
        return ['code' => 0, 'data' => '', 'msg' => $this->getError()];
    } else {
        return ['code' => 1, 'data' => $flag, 'msg' => '成功'];
    }
}
...
//生成网址的二维码 返回图片地址
function Qrcode($token='123456', $url, $size = 8){
    $md5 = md5($token);
