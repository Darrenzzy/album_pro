...
class AjaxFileUpload
{
    ...
    public function uploadImg(){
        $fileName = input('param.filename');//获取文件名
        $position = input('param.position');//上传路径
        return $this->uploader($fileName,$position);
     ...
     */
    public function uploader($fileName,$position) {
        $files = array();
        ...
        if($_FILES[$fileName]['error']==0){
            ...
            if(is_uploaded_file($_FILES[$fileName]['tmp_name'])) {
                ...
                if(trim($files['info'])==""){
                    ...
                    $path = ROOT_PATH.'public'.DS.'uploads/'.$position.'\/'. @date("Ymd"). '\/';
                    ///确保目录可写
                    if($this->ensure_writable_dir($path)){
                        ...
                        if ($ret === false) {
                            ...
                            $files['path'] = '/uploads/'.$position.'\/'.@date("Ymd").'\/'.$name;        //存图片路径
